...
- [x] T001 Create base Docusaurus project structure in `docusaurus/`
- [x] T002 Configure `docusaurus.config.js` for basic site metadata and routing in `docusaurus/docusaurus.config.js`
- [x] T003 Set up GitHub Pages deployment in `.github/workflows/docusaurus-deploy.yml`
- [x] T004 Initialize FastAPI project structure for RAG backend in `api/`
...
### Implementation Tasks:
- [x] T005 Configure Neon Serverless Postgres connection in `api/config.py`
- [x] T006 Configure Qdrant Cloud Free Tier connection in `api/config.py`
- [x] T007 Implement basic book content structure with example chapters in `docusaurus/docs/`
- [x] T008 Initial setup for OpenAI Agents/ChatKit SDKs in `api/services/rag_service.py`
- [x] T009 Implement basic text extraction and chunking logic for embedding pipeline in `api/services/embedding_pipeline.py`
...
### Implementation Tasks:
- [x] T010 [US1] Create RAG chatbot frontend component in `docusaurus/src/components/RAGChatbot.js`
- [x] T011 [US1] Integrate RAG chatbot component into Docusaurus layout (e.g., `docusaurus/src/theme/Layout.js`)
- [x] T012 [P] [US1] Implement FastAPI endpoint for general RAG queries in `api/routers/rag.py`
- [x] T013 [P] [US1] Implement FastAPI endpoint for RAG queries with selected text context in `api/routers/rag.py`
- [x] T014 [US1] Develop logic for retrieving relevant documents from Qdrant in `api/services/rag_service.py`
- [x] T015 [US1] Develop logic for generating responses using OpenAI Agents/ChatKit in `api/services/rag_service.py`
- [x] T016 [US1] Implement logic to provide citations from book content in `api/services/rag_service.py`
- [x] T017 [US1] Integrate frontend with RAG chatbot API endpoints in `docusaurus/src/components/RAGChatbot.js`
...
### Implementation Tasks:
- [x] T018 [US2] Integrate Better-Auth.com for signup/signin into Docusaurus frontend in `docusaurus/src/components/Auth.js`
